...
def test_best17_a_tlt_hedge_metrics_regression_baseline(us_data_dir):
    ...
    momentum_hedge_overlay/tlt_hedge/best17_a razem. Poprzednia baseline (przed poprawka rankingu
    w histerezie): cagr=0.1301, max_drawdown=-0.2370, sharpe=0.897."""
    ...
    assert metrics["cagr"] == pytest.approx(0.1434, abs=0.01)
    assert metrics["max_drawdown"] == pytest.approx(-0.1808, abs=0.01)
    assert metrics["sharpe"] == pytest.approx(0.974, abs=0.05)
