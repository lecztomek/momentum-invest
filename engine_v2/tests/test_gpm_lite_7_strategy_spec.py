"""
Testy dla strategies_v2/gpm_lite_7/ ("gpm_lite_7" - uproszczona wersja "gpm" na 7 aktywach
ryzykownych zamiast 13, user: "zachowac mechanike GPM przy mniejszej liczbie tickerow, nizszym
turnoverze i prostszym wdrozeniu") - w odroznieniu od test_gpm_components.py (bloki na
SYNTETYCZNYCH danych), tu ladujemy FAKTYCZNY strategy_spec.json i sprawdzamy wiring + end-to-end
na realnych danych. Zero nowego kodu bloku - identyczna architektura co `gpm`
(`momentum_avg_month_end`/`corr_to_basket_month_end`/`momentum_times_decorrelation`/
`gpm_breadth_protective_split`), tylko mniejsze uniwersum.

Uruchomienie: .venv/bin/pytest engine_v2/tests/test_gpm_lite_7_strategy_spec.py -v
"""
...
GPM_LITE_7_SPEC_PATH = REPO_ROOT / "strategies_v2" / "gpm_lite_7" / "strategy_spec.json"


def _load_spec() -> StrategySpec:
    return StrategySpec.load(GPM_LITE_7_SPEC_PATH)


def test_gpm_lite_7_spec_is_valid():
    spec = _load_spec()
    assert spec.validate() == []


def test_gpm_lite_7_spec_resolves_all_blocks():
    spec = _load_spec()
...
def test_gpm_lite_7_risky_universe_has_7_assets_and_2_protective():
    spec = _load_spec()
    ...
    assert len(risky) == 7
    assert set(risky) == {"vt.us", "qqq.us", "vwo.us", "vnq.us", "dbc.us", "gld.us", "xle.us"}
...
def test_gpm_lite_7_full_chain_on_real_data(us_data_dir):
    spec = _load_spec()
    ...
    # ekspozycja ryzykowna (co najmniej jedno z 7 aktywow ryzykownych > 0)
...
def test_gpm_lite_7_metrics_regression_baseline(us_data_dir):
    """Zamrozony wynik na realnych danych (2026-07-12, PRZED podatkiem) - lapie regresje w
    blokach uzywanych przez gpm_lite_7 (dokladnie te same co gpm: momentum_avg_month_end,
    ...
    assert metrics["cagr"] == pytest.approx(0.0564, abs=0.01)
    assert metrics["max_drawdown"] == pytest.approx(-0.1363, abs=0.01)
    assert metrics["sharpe"] == pytest.approx(0.649, abs=0.05)
