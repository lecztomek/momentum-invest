"""
Testy dla strategies_v2/gtaa_agg3_mid/ i strategies_v2/gtaa_agg6_mid/ - user: "Dorobmy taka
strategie AGG... wydaje mi sie ze bedzie sensowniejsza do mixu". Mechanika IDENTYCZNA z juz
istniejacym gtaa_agg3/gtaa_agg6 (ten sam blok gtaa_trend_bond_reroute, zero nowego kodu) - jedyna
roznica to uniwersum: bez IJR/EFA (trudne do zmapowania na UK/XTB), zamiast tego uniwersum
gpm_mid_10 (SPY/QQQ/VWO/VNQ/DBC/GLD/HYG/LQD/TLT/XLE + IEF).

Uruchomienie: .venv/bin/pytest engine_v2/tests/test_gtaa_mid_strategy_specs.py -v
"""

import json
from pathlib import Path

import pytest

from engine_v2.pipeline import resolve_blocks, run_strategy_pipeline
from engine_v2.spec import StrategySpec

REPO_ROOT = Path(__file__).resolve().parents[2]
STRATS = REPO_ROOT / "strategies_v2"

VARIANTS = {"gtaa_agg3_mid": 3, "gtaa_agg6_mid": 6}
_MID_UNIVERSE_RISKY = {
    "spy.us", "qqq.us", "vwo.us", "vnq.us", "dbc.us", "gld.us", "hyg.us", "lqd.us", "tlt.us", "xle.us",
}


def _load_spec(name: str) -> StrategySpec:
    return StrategySpec.load(STRATS / name / "strategy_spec.json")


@pytest.mark.parametrize("name", VARIANTS)
def test_gtaa_mid_spec_is_valid(name):
    spec = _load_spec(name)
    assert spec.validate() == []


@pytest.mark.parametrize("name", VARIANTS)
def test_gtaa_mid_spec_resolves_all_blocks(name):
    spec = _load_spec(name)
    resolved = resolve_blocks(spec)
    for block_type in spec.blocks:
        assert block_type in resolved


@pytest.mark.parametrize("name", VARIANTS)
def test_gtaa_mid_universe_excludes_ijr_efa(name):
    """Cala racja bytu tego wariantu - bez IJR/EFA (trudne do zmapowania na UK/XTB)."""
    spec = _load_spec(name)
    risky = set(spec.universe) - {"ief.us"}
    assert risky == _MID_UNIVERSE_RISKY
    assert "ijr.us" not in spec.universe
    assert "efa.us" not in spec.universe


@pytest.mark.parametrize("name, top_n", VARIANTS.items())
def test_gtaa_mid_top_n_matches_number_of_equal_weights(name, top_n):
    spec = _load_spec(name)
    assert spec.base_params["selector"]["top_n"] == top_n
    weights = spec.base_params["alpha_weighting"]["weights"]
    assert len(weights) == top_n
    assert sum(weights) == pytest.approx(1.0, abs=1e-9)


@pytest.mark.parametrize("name, top_n", VARIANTS.items())
def test_gtaa_mid_full_chain_on_real_data(name, top_n, us_data_dir):
    spec = _load_spec(name)
    spec.base_params["data_loader"]["data_dir"] = str(us_data_dir)

    final_portfolio = run_strategy_pipeline(spec)

    assert final_portfolio["date"].is_monotonic_increasing
    assert len(final_portfolio) > 12
    for weights_json in final_portfolio["weights_used_json"]:
        weights = json.loads(weights_json)
        assert sum(weights.values()) == pytest.approx(1.0, abs=1e-6)

    bond_fallback = spec.base_params["portfolio_risk_engine"]["bond_fallback_asset"]
    risky = set(spec.universe) - {bond_fallback}
    saw_full_bond = saw_risky_exposure = saw_mixed_slot = False
    for weights_json in final_portfolio["weights_used_json"]:
        weights = json.loads(weights_json)
        bond_weight = weights.get(bond_fallback, 0.0)
        risky_weight = sum(weights.get(t, 0.0) for t in risky)
        held_risky = sum(1 for t in risky if weights.get(t, 0.0) > 1e-9)
        if bond_weight >= 1.0 - 1e-9:
            saw_full_bond = True
        if risky_weight > 1e-9:
            saw_risky_exposure = True
        if bond_weight > 1e-9 and risky_weight > 1e-9:
            saw_mixed_slot = True
        assert held_risky <= top_n

    assert saw_full_bond
    assert saw_risky_exposure
    assert saw_mixed_slot


def test_gtaa_agg3_mid_metrics_regression_baseline(us_data_dir):
    """Zamrozony wynik na realnych danych (2026-07-14, PRZED podatkiem)."""
    from engine_v2.backtest_engine import daily_equity_curve
    from engine_v2.blocks.data_loader import REGISTRY as LOADER_REGISTRY
    from engine_v2.metrics import compute_metrics

    spec = _load_spec("gtaa_agg3_mid")
    spec.base_params["data_loader"]["data_dir"] = str(us_data_dir)

    final_portfolio = run_strategy_pipeline(spec)
    market_data = LOADER_REGISTRY["stooq_csv"](spec.universe, {"data_dir": str(us_data_dir), "frequency": "monthly"})
    equity_curve = daily_equity_curve(final_portfolio, market_data.prices, {})
    metrics = compute_metrics(equity_curve, final_portfolio, {})

    assert metrics["cagr"] == pytest.approx(0.0859, abs=0.01)
    assert metrics["max_drawdown"] == pytest.approx(-0.2113, abs=0.01)
    assert metrics["sharpe"] == pytest.approx(0.661, abs=0.05)


def test_gtaa_agg6_mid_metrics_regression_baseline(us_data_dir):
    """Zamrozony wynik na realnych danych (2026-07-14, PRZED podatkiem)."""
    from engine_v2.backtest_engine import daily_equity_curve
    from engine_v2.blocks.data_loader import REGISTRY as LOADER_REGISTRY
    from engine_v2.metrics import compute_metrics

    spec = _load_spec("gtaa_agg6_mid")
    spec.base_params["data_loader"]["data_dir"] = str(us_data_dir)

    final_portfolio = run_strategy_pipeline(spec)
    market_data = LOADER_REGISTRY["stooq_csv"](spec.universe, {"data_dir": str(us_data_dir), "frequency": "monthly"})
    equity_curve = daily_equity_curve(final_portfolio, market_data.prices, {})
    metrics = compute_metrics(equity_curve, final_portfolio, {})

    assert metrics["cagr"] == pytest.approx(0.0701, abs=0.01)
    assert metrics["max_drawdown"] == pytest.approx(-0.1729, abs=0.01)
    assert metrics["sharpe"] == pytest.approx(0.707, abs=0.05)


@pytest.mark.parametrize("name", VARIANTS)
def test_gtaa_mid_uk_mapping_end_to_end(name, us_data_dir, uk_data_dir):
    """"Ostateczny test" - pelne pokrycie (11/11 tickerow zmapowanych) - zero mismatch oczekiwane,
    to cala racja bytu tego wariantu wzgledem gtaa_agg3/gtaa_agg6 (ktore uzywaja IJR/EFA)."""
    from engine_v2.run_spec import RunSpec
    from engine_v2.run_spec_runner import run
    from engine_v2.test_spec import TestSpec

    strategy_dir = STRATS / name
    test_spec = TestSpec.load(strategy_dir / "test_spec.json")
    test_spec.uk_mapping.uk_data_dir = str(uk_data_dir)

    original_text = (strategy_dir / "test_spec.json").read_text(encoding="utf-8-sig")
    test_spec.save(strategy_dir / "test_spec.json")
    try:
        run_spec = RunSpec.load(strategy_dir / "run_spec.json")
        run_spec.mode = "final"
        result = run(run_spec, strategy_dir)
    finally:
        (strategy_dir / "test_spec.json").write_text(original_text, encoding="utf-8")

    uk_result = result["uk_mapping"]
    assert uk_result["diagnostics"]["mismatch_pct"] == 0.0
    assert uk_result["comparison"]["monthly_return_correlation"] > 0.9
    assert abs(uk_result["comparison"]["cagr_gap"]) < 0.05
    assert abs(uk_result["comparison"]["max_drawdown_gap"]) < 0.05
